...
/*tumsim--- the program of subclone simulator for data generator file(*.sim or
 **_sunchged.idx) Several subclones were usually found in cancer cells. This
 *program can run multiple times, it can respectively realizes to set variation
 *of sites of all subclones . In this, considering inheritance  and mutual
 *exclusivity of the variation between different subclones. */
...
float HD_POS =
    0.5; // position in reference for high density area of ervry turmor
  ...
  *p = '\0';
  ...
  while (!feof(fp) && (c = fgetc(fp))) {
    if (isalpha(c) || c == '-' || c == '.') {
      ...
      if (seq->s[l - 1] == 'A' || seq->s[l - 1] == 'C' ||
          seq->s[l - 1] == 'G' || seq->s[l - 1] == 'T')
        n++;
    }
  }
  seq->s[l] = 0;
...
/**********************************************/
int set_pos_sim(mutseq_t *hap1, mutseq_t *hap2, seq_t *nochged,
                int k) { // return chged num
  int deleting = 0;
  mutseq_t *ret[2];
    ...
    if (zrand() < DEL_RATE) {  // deletion
      ...
      while (zrand() < INDEL_EXTEND && k < max && c0 < 4 && c1 < 4 &&
        ...
        if (deleting < 3)
          ...
          ret[0]->s[k] = ret[1]->s[k] |= DELETE | GTYPEBB;
        chgnum++;
        nochged->s[k] |= 4;
        k++;
        c0 = ret[0]->s[k];
...
/*******************************************************/
void set_mut_sim(seq_t *seq, mutseq_t *hap1, mutseq_t *hap2, seq_t *nochged,
                 mutseq_t *norab) {
  mutseq_t *ret[2];
  int num, j, k, n, m, max;
  double r;
  ...
  m = 0;
  max = ret[0]->l;
  num = (int)(ret[0]->v * SV_MUT_RATE) - HD_NUM;
  hdbeg = max * HD_POS; // High density area begenning
  printf("Enter set_mut_sim max=%d   num=%d \n", max, num);
  n = 0;
  while (n < num) { // set mutation and not normal AB mode, not High density
                    // area
    k = zrand() * max;
    c0 = ret[0]->s[k];
    c1 = ret[1]->s[k];
    while (c0 >= 4 || c1 >= 4 || nochged->s[k] ||
           ((HD_LEN > 0) && (k > hdbeg && k < hdbeg + HD_LEN))) {
      k = zrand() * max;
      c0 = ret[0]->s[k];
      c1 = ret[1]->s[k];
    }
  ...
  while (n < HD_NUM) {            // set mutation in High density area
    k = hdbeg + zrand() * HD_LEN; // random in High density area
    c0 = ret[0]->s[k];
    c1 = ret[1]->s[k];
    while (c0 >= 4 || c1 >= 4 || nochged->s[k]) {
      k = hdbeg + zrand() * HD_LEN;
      c0 = ret[0]->s[k];
      c1 = ret[1]->s[k];
    }
    nochged->s[k] |= 4;
  ...
  printf(
      "\tEnter LOH_NOR_AB_RATE set:  LOH_NOR_AB num=%d   nor_AB_tab size=%d\n",
      num, norab->l);
  for (j = 0; j < num; j++) { // mutation from normal AB mode
    ...
    m = 0;
    while (m < 10 &&
           ((c0 & 0x30) > GTYPEAB || (c1 & 0x30) > GTYPEAB || nochged->s[n])) {
    ...
    if (m < 10) {
      ...
      printf("\tFailed to Search NOR_AB_TAB:\n\t\thas seted num=%d    try num "
             "= %d   rand tab pos=%d   reference pos=%d\n",
             j, m, k, n);
  }
...
/*******************************************************/
int addinsert_iterm(indel_t *indelp, int idno, int idbeg, int idlen, int idtype,
...
/*******************************************************/
void init_del_sim(seq_t *seq, mutseq_t *hap1, mutseq_t *hap2, seq_t *nochged,
                  mutseq_t *norab, indel_t *indelp, char *simfile) {
  ...
  if (!indelfp || !LONG_INDEL)
    return; // No Long Insert/Delete and Repeating !
  ...
  if (inbasesimfp) {
    ...
    while (fscanf(inbasesimfp, "%d %d %d %d %d %d %s\n", &idno, &idbeg, &idlen,
                  &idtype, &idnum, &idvar, idstr) != EOF) {
      addnum = idno;
      addinsert_iterm(indelp, idno, idbeg, idlen, idtype, idnum, idvar, idstr,
                      &n2);
      n++;
    } // while (reading file .....)
    printf("read end of base *.sim.idx:: Long insert num=%d    long insert "
           "len=%d    Long insert table size=%d\n",
           n, n2, indelp->l);
    fclose(inbasesimfp);
  }
  ...
  while (fscanf(indelfp, "%d %d %d %d %d %d %s\n", &idno, &idbeg, &idlen,
    ...
    if (idlen < 0) { // long delete from long indel file!!!
      idlen = -idlen;
      if (idbeg < 0 || idbeg + idlen - 1 >= REF_LEN)
        idtype = -1;
      switch (idtype) {
      case 0:
        break;
      case 1:
        ...
        for (int i = 0; i < idlen; i++) {
          ...
          if (indelmk == LONGINSERT)
            ret[h]->s[idbeg + i] = (ret[h]->s[idbeg + i] & NOTDELETE) | GTYPEAB;
          else
            ret[h]->s[idbeg + i] |= DELETE | GTYPEAB;
        }
        n1 = n1 + idlen;
        del_no++;
        break;
      ...
      case 3:
        for (int i = 0; i < idlen; i++) {
          ...
          indelmk = ret[h]->s[idbeg + i] & LONGINSERT;
          if (indelmk == LONGINSERT)
            ret[h]->s[idbeg + i] = (ret[h]->s[idbeg + i] & NOTDELETE) | GTYPEBB;
          else
            ret[h]->s[idbeg + i] |= DELETE | GTYPEBB;
          h = 1;
          indelmk = ret[h]->s[idbeg + i] & LONGINSERT;
          if (indelmk == LONGINSERT)
            ret[h]->s[idbeg + i] = (ret[h]->s[idbeg + i] & NOTDELETE) | GTYPEBB;
      ...
      case 1:
        ...
        if (indelmk == DELETE)
          ret[h]->s[idbeg] |= (LONGINSERT | GTYPEAB | (addnum << 8));
        else
          ret[h]->s[idbeg] |= (LONGINSERT | INSERT | GTYPEAB | (addnum << 8));
        break;
      case 2:
        h = 1;
        indelmk = ret[h]->s[idbeg] & DELETE;
      ...
      case 3:
        ...
        ret[h]->s[idbeg] = (ret[h]->s[idbeg] & 0x03);
        if (indelmk == DELETE)
          ret[h]->s[idbeg] |= (LONGINSERT | GTYPEBB | (addnum << 8));
        else
          ret[h]->s[idbeg] |= (LONGINSERT | INSERT | GTYPEBB | (addnum << 8));
        h = 1;
        indelmk = ret[h]->s[idbeg] & DELETE;
        ret[h]->s[idbeg] = (ret[h]->s[idbeg] & 0x03);
        if (indelmk == DELETE)
      ...
      if (idtype > 0) {
        ...
        if (idvar == 0) { // repeat without var genos
          for (i = 0; i < idnum; i++)
            addinsert_iterm(indelp, addnum, idbeg, idlen, idtype, idnum, idvar,
                            idstr, &n2);
          ...
          while (res_num > 4) {
            ...
            for (i = 0; i < k; i++)
              addinsert_iterm(indelp, addnum, idbeg, idlen, idtype, idnum,
                              idvar, idstr, &n2);
            res_num = res_num - k;
...
void print_simfile(mutseq_t *hap1, mutseq_t *hap2, char *refname,
  ...
  for (i = 0; i < hap1->l; ++i) {
    ...
    if ((hap1->s[i] & mutmsk) == SUBSTITUTE ||
        (hap2->s[i] & mutmsk) == SUBSTITUTE) {
      if (((hap1->s[i] & 0x30) == GTYPEAB) || ((hap2->s[i] & 0x30) == GTYPEAB))
        fprintf(outsABp, "%10u\t%10u\t%10u\n", i, hap1->s[i], hap2->s[i]);
      if (((hap1->s[i] & 0x30) == GTYPELOH) ||
          ((hap2->s[i] & 0x30) == GTYPELOH))
  ...
  if (n > 0) {
    ...
    printf("\tref2_len=%d  ref2 var num = %d Var rate = %.10f\n\n", n1, cnt1,
           1.0 * cnt1 / n1);
    printf("\tRef_len =%d  all  var num = %d Var rate = %.10f chged_gch num= "
           "%d\n\n",
    ...
    fprintf(outresult, "\n ref2_len=%d  ref2 var num = %d Var rate = %.10f\n\n",
            n1, cnt1, 1.0 * cnt1 / n1);
    fprintf(outresult,
            "\n Ref_len =%d  all  var num = %d Var rate = %.10f chged_files = "
            "%d\n\n",
            REF_LEN, n, 1.0 * n / REF_LEN, chg_no);
  }
  return;
...
static int simu_usage() {
  ...
  printf("Usage:   tumsim [options] <ref.fa> <base.sim> <nor_AB.idx> "
         "<subclone.sim>\n");
  ...
  printf("         -b FLOAT      	LOH BB rate in mutation [%10.6f]\n",
         LOH_BB_RATE); // 0.5
  printf(
      "         -A FLOAT      	mutation rate of LOH in normal AB [%12.10f]\n",
  ...
  printf("         -l int       	length of high desity [%d]\n",
         HD_LEN); // 100000
  printf("         -I <indelpos.txt>  	input long indel set file. generating "
         "<subclone.sim.idx> for 'ReadGen'\n");
  printf("         -N <other_chged.idx> 	input changed positions in "
         "other subclone(may multi-choices)\n");
  printf("         -C <*_chged.idx>  		output changed positions in "
         "this turmor\n");
...
int main(int argc, char *argv[]) {
  ...
  indelf[0] = '\0';
  cc = 0;
  while ((c = getopt(argc, argv, "r:R:X:D:B:b:A:n:p:l:I:N:C:o:")) >= 0) {
    switch (c) {
    case 'r':
      SV_MUT_RATE = atof(optarg);
      if (SV_MUT_RATE < 0 || SV_MUT_RATE > 1)
        cc = 1;
      break;
    case 'R':
    ...
    case 'B':
      ...
      break;
    case 'b':
      LOH_BB_RATE = atof(optarg);
      if (LOH_BB_RATE < 0 || LOH_BB_RATE > 1)
        cc = 1;
      break;
    case 'A':
      LOH_NOR_AB_RATE = atof(optarg);
      if (LOH_NOR_AB_RATE < 0 || LOH_NOR_AB_RATE > 1)
        cc = 1;
      break;
    case 'n':
      HD_NUM = atoi(optarg);
      if (HD_NUM < 0 || HD_NUM > HD_LEN)
        cc = 1;
      break;
    case 'p':
      HD_POS = atof(optarg);
      if (HD_POS < 0 || HD_POS > 1)
        cc = 1;
      break;
    case 'l':
      HD_LEN = atoi(optarg);
    ...
    case 'I':
      ...
      break;
    case 'N':
      strcpy(INOCHGFs[OTHER_CHGED], optarg);
      OTHER_CHGED++;
      break;
    case 'C':
      OUT_CHGED = 1;
      strcpy(outchgf, optarg);
      break;
    case 'o':
