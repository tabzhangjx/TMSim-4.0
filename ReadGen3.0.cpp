/*readgen--- the program of reads generating for single or paired_end
 *file(*_left.fq, *_right.fq) Based on *.sim files that are generated from
 *normal and subclones. It form the simulation environment including all kinds
 *of variations and complete reads random sampling. Last, it can random generate
 *two files of left.fq and right.fq that exit in positive and reverse
 *chain,respectively.*/
...
int KEEP_N = 0; // keep "N" in seq_read
int SINGLE_READ = 0;
...
int addinsert_iterm(indel_t *indelp, int idno, int idbeg, int idlen, int gtype,
                    int idnum, int idvar, char *idstr) {
  int pos;
  pos = indelp->l;
  if (pos + 1 >= indelp->max) {
...
/*******************************************************/
void init_sim(seq_t *seq, mutseq_t *hap1, mutseq_t *hap2, char *indelfile,
              indel_t *indelp) {
  int fret;
...
/*******************************************************/
void press_sim(mutseq_t *hap1, mutseq_t *hap2, seq_t seq[2][2],
               indel_t *indelp) {
  int i, j, k, n, m;
  seq_t *s[2];
  mut_t msk, c;
  ...
  for (i = 0; i < hap1->l; ++i) {
    ...
    if (msk != DELETE && (c & 0xf) <= (KEEP_N ? 4 : 3)) {
      if ((msk == 0 && ((c & LONGINSERT) != LONGINSERT)) || msk == SUBSTITUTE) {
        put_gch(s[0], s[1], (c & 0xf));
        if (msk == SUBSTITUTE)
          m++;
      } else if ((c & LONGINSERT) == LONGINSERT) { // long insert
        int in_no, pos, k, cc, loopmk;
        // this mask can only be INSERT
        if (msk != 0)
          // there may be a bug here
          // if a gene is first deleted, then is chosen to be the position to
          // long insert, then the original gene in its 0x3 should still be
          // diposited, but that is not ture here
          put_gch(s[0], s[1], (c & 0x3));
        m++;
        pos = ((c << 4) >> 12); // c >> 8 ?
        loopmk = 0;
        for (j = 0; j < indelp->l; j++) {
          if (indelp->s[j].indel_no == pos) {
            ...
            for (k = 0; k < in_no; k++) {
              ...
              // ignore unknown gene in inserting snippet, why
              if (cc < 4) {
                put_gch(s[0], s[1], cc);
                m++;
              }
            }
            loopmk = 1; // why not just break here
          } else if (loopmk)
            break;
        }
...
// output the Chrstring of current bady
void printChrstr(char *refname, seq_t seq[2][2], char *ref_ChrName) {
  int i, n;
  seq_t *s0, *s1;
  char ChrFileName[100], c;
  s0 = &seq[0][0]; // Father Chr string
  s1 = &seq[1][0]; // Mather Chr string
  // output father ChrString F-ref.fa
  FILE *outChrF;
  // this name is not suitable for argv including folder's name
  sprintf(ChrFileName, "F-%s", refname);
  outChrF = FileOpen(ChrFileName, "wt");
  fprintf(outChrF, ">%s\n", ref_ChrName);
  n = s0->l;
  i = 0;
  while (i + 50 < n) {
  ...
  // output father ChrString M-ref.fa
  sprintf(ChrFileName, "M-%s", refname);
  outChrF = FileOpen(ChrFileName, "wt");
  fprintf(outChrF, ">%s\n", ref_ChrName);
  n = s1->l;
  i = 0;
...
static int simu_usage() {
  ...
  printf("Options: -d INT		outer distance between the two ends of "
         "paired_end reads [%d]\n",
         MAX_DIS);
  ...
  printf("         -I <subclonde.sim.idx>   long insert table file from "
         "'TumSim'\n");
...
int main(int argc, char *argv[]) {
  ...
  while ((c = getopt(argc, argv, "d:s:l:r:c:e:I:O:n:Sk")) >= 0) {
    ...
    case 'd':
      MAX_DIS = atoi(optarg);
      if (MAX_DIS < LREAD_LEN + RREAD_LEN)
        cc = 1;
      break;
    case 's':
      DALT = atoi(optarg);
      if (DALT < 0 || DALT >= MAX_DIS / 2)
        cc = 1;
      break;
    case 'l':
      LREAD_LEN = atoi(optarg);
      if (LREAD_LEN <= 0)
        cc = 1;
      break;
    case 'r':
      RREAD_LEN = atoi(optarg);
      if (RREAD_LEN <= 0)
        cc = 1;
      break;
    case 'c':
      COVER_RATE = atof(optarg);
      if (COVER_RATE <= 0)
        cc = 1;
      break;
    case 'e':
  ...
  // output the Chrstring of current bady
  printChrstr(ingenf, gseq, ChrName);

  // generate pair_end reads
  if (SINGLE_READ)
    gen_single_read(gseq, ChrName);
  else
    gen_pair_reads(gseq, ChrName);
...
/**********************************************/
static int out_read(int l_beg, int r_end, char *l_readmk, char *r_readmk,
                    int pr_no, seq_t qseq[2][2], int n1, int n2,
                    char *ref_name) {
  ...
  for (j = 0; j < LREAD_LEN; j++) {
    ...
    if (n2 == 0)
      left[j] = qseq[n1][0].s[l_beg + j];
    else
      left[j] = qseq[n1][1].s[r_end - j];
  }
  left[LREAD_LEN] = '\0';
  if (err_num > 0) {
    for (j = 0; j < err_num; j++) {
  ...
  for (j = 0; j < RREAD_LEN; j++) {
    ...
    if (n2 == 0)
      right[j] = qseq[n1][1].s[r_end - j];
    else
      right[j] = qseq[n1][0].s[l_beg + j];
  }
  right[RREAD_LEN] = '\0';
  if (err_num > 0) {
    for (j = 0; j < err_num; j++) {
  ...
  if (n2 == 0) {
    l_zbeg = l_beg;
    r_zbeg = r_end - RREAD_LEN + 1;
  } else {
    l_zbeg = r_end - RREAD_LEN + 1;
    r_zbeg = l_beg;
  }

  fprintf(outleft, "@%s_%d_%d_%d_%d_%d_0:0:0_0:0:0_%x/1\n", ref_name, l_beg + 1,
          r_end + 1, l_zbeg + 1, n1, n2, pr_no);
  fprintf(outleft, "%s\n", left);
  fprintf(outleft, "+\n%s\n", l_readmk);

  fprintf(outright, "@%s_%d_%d_%d_%d_%d_0:0:0_0:0:0_%x/2\n", ref_name,
          l_beg + 1, r_end + 1, r_zbeg + 1, n1, n2, pr_no);
  fprintf(outright, "%s\n", right);
  fprintf(outright, "+\n%s\n", r_readmk);
...
void beg_end(int msk, int &beg, int &end, int len) {
  ...
  case 1:
    ...
    break;
  case 2:
    beg = end - pair_reads_dis + 1;
    break;
  case 0: // k = (int)(1.0*zrand()*len);
    while (1) {
      beg = zrand() * len;
      if (beg + pair_reads_dis - 1 < len)
        break;
    }
    end = beg + pair_reads_dis - 1;
    break;
...
int gen_pair_reads(seq_t qseq[2][2], char *ref_n) {
  ...
  for (i = 0; i < LREAD_LEN; i++)
    l_readmk[i] = 'I';
  l_readmk[LREAD_LEN] = '\0';
  for (i = 0; i < RREAD_LEN; i++)
    r_readmk[i] = 'I';
  ...
  printf("\tpair_end reads total = [%d], Error pair reads =[%d] error "
         "rate=[%f] in ref1 + ref2.\n",
         pr_no, err_preads, 1.0 * err_preads / pr_no);
  printf("\ttotal gchar = [%d] error gchar num=[%d]  total error rate=[%f].\n",
         m, err_num, 1.0 * err_num / m);
  fprintf(outresult,
          "pair_end reads total = [%d], Error pair reads =[%d] error rate=[%f] "
          "in ref1 + ref2.\n",
          pr_no, err_preads, 1.0 * err_preads / pr_no);
  fprintf(outresult,
          "total gchar = [%d] error gchar num=[%d]  total error rate=[%f].\n",
          m, err_num, 1.0 * err_num / m);
...
/**********************************************/
static int out_single_read(int beg, char *readmk, int pr_no, seq_t qseq[2][2],
                           int n1, int n2, char *ref_name) {
  char left[LREAD_LEN + 1], ch;
  ...
  for (j = 0; j < LREAD_LEN; j++) {
    ...
    if (n2 == 0)
      left[j] = qseq[n1][0].s[beg + j];
    else
      left[j] = qseq[n1][1].s[beg + LREAD_LEN - 1 - j];
  }
  left[LREAD_LEN] = '\0';
  if (err_num > 0) {
    if (ERROR_RATE > 0.03 && LREAD_LEN > 1000) {
      for (j = 0; j < err_num; j++) {
        n = (int)(zrand() * LREAD_LEN);
        k = gch_to_num(left[n]);
        k = (k + (int)(zrand() * 3.0 + 1)) & 3;
        left[n] = num_to_gch(k);
      }
...
int gen_single_read(seq_t qseq[2][2], char *ref_n) {
  ...
  READS_NUM = COVER_RATE * a_len / LREAD_LEN;
  fprintf(
      outresult,
      "\ngen_single_read():\nSingle read num==[%d]   total cover rate=[%f]\n",
      READS_NUM, COVER_RATE);
  pr_no = 0;
  err_num = 0;
  err_preads = 0;

  while (pr_no < READS_NUM) {
    m = zrand() < 0.5 ? 0 : 1;
    k = zrand() < 0.5 ? 0 : 1;
    do {
  ...
  m = pr_no * LREAD_LEN;
  fprintf(outresult,
          "single reads total = [%d], Error reads =[%d] error rate=[%f] in "
          "ref1 + ref2.\n",
          pr_no, err_preads, 1.0 * err_preads / pr_no);
  fprintf(outresult,
          "total gchar = [%d] error gchar num=[%d]  total error rate=[%f].\n",
